...
func upgrade_skill():
	...
	effect_dot.ticks_per_second = 3.0 + (0.5*skill_level)
...
func get_desc() -> String:
 ...
 %[(effect_dot.damage_per_tick*effect_dot.ticks_per_second+0.5*effect_dot.damage_multiplier+0.05), effect_dot.duration+1.2, cooldown-0.5, skill_level]
	return "[b][color=#dbc4a6]other spells can bring its full potential[/color][/b]\n\n[table=2][cell]damage per second: [/cell][cell]%s[/cell]\n[cell]duration: [/cell][cell]%s[/cell]\n[cell]cooldown: [/cell][cell]%s[/cell]\n[cell]skill level: [/cell][cell]%s[/cell][/table]" \
...
func get_skill_name() -> String:
	return "[wave amp=20.0 freq=8.0x connected=0][rainbow freq=2.0 sat=0.4 val=1.0 speed=0.5]Whirlwind[/rainbow][/wave]"
