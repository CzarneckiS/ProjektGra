...
var projectile = preload("res://resources/boss_basic_projectile.tres")
...
var melee_attack_vfx = preload("res://vfx/melee_attack_slash/boss_attack_slash_vfx.tres")
...
func _ready() -> void:
	...
	navigation_agent_2d.radius = 30 #im wieksza liczba tym bardziej unika kolizji
...
var chunks : Array = []
func prepare_targeting_chunks() -> void:
	for i in range(chunk_row_count):
		chunks.append([])
...
func add_to_heatmap(target_position: Vector2):
	...
	if chunk_x+1 in range(chunk_row_count) and chunk_y+1 in range(chunk_row_count):
		chunks[chunk_x+1][chunk_y+1] += 1
func find_best_chunk() -> Vector2:
	var temp_value = 0
	var biggest_value = 0
	var potential_chunks = []
	for i in chunks.size():
		for j in chunks.size():
			temp_value += chunks[i][j]
			if i-1 in range(chunk_row_count):
				temp_value += chunks[i-1][j]
			if i+1 in range(chunk_row_count):
				temp_value += chunks[i+1][j]
			if j-1 in range(chunk_row_count):
				temp_value += chunks[i][j-1]
			if j+1 in range(chunk_row_count):
				temp_value += chunks[i][j+1]
			if i-1 in range(chunk_row_count) and j-1 in range(chunk_row_count):
				temp_value += chunks[i-1][j-1]
			if i-1 in range(chunk_row_count) and j+1 in range(chunk_row_count):
				temp_value += chunks[i-1][j+1]
			if i+1 in range(chunk_row_count) and j-1 in range(chunk_row_count):
				temp_value += chunks[i+1][j-1]
			if i+1 in range(chunk_row_count) and j+1 in range(chunk_row_count):
				temp_value += chunks[i+1][j+1]
			if temp_value > biggest_value:
				potential_chunks.clear()
				biggest_value = temp_value
				@warning_ignore("integer_division")
				potential_chunks.append(Vector2(i*chunk_size+global_position.x-half_map_size + chunk_size/2, j*chunk_size+global_position.y-half_map_size + chunk_size/2))
			elif temp_value == biggest_value:
				@warning_ignore("integer_division")
				potential_chunks.append(Vector2(i*chunk_size+global_position.x-half_map_size + chunk_size/2, j*chunk_size+global_position.y-half_map_size + chunk_size/2))
			temp_value = 0
	var best_position: Vector2
	if biggest_value == 0:
		best_position = global_position.direction_to(Globals.player_position) * 800
	else:
		best_position = potential_chunks[randi()%potential_chunks.size()]
	return best_position
#func _draw():
	#for i in chunks.size():
		#for j in chunks.size():
			#draw_circle(Vector2(i*chunk_size+global_position.x-half_map_size + chunk_size/2, j*chunk_size+global_position.y-half_map_size + chunk_size/2),10,Color())
...
func cast_dash(unit, target_destination : Vector2):
